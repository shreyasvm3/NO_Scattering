...
E0r117BW100 = [E0calc(1.17,zk,1.5,100,200)*htoev for zk in zrange]
E0r16BW100 = [E0calc(1.6,zk,1.5,100,200)*htoev for zk in zrange]
E0z16BW100 = [E0calc(rk,1.6,1.5,100,200)*htoev for rk in rrange]

#Calculate reference energy at R = 1.6 A and Z = 6.02 A 
ErefBW100 = E0calc(1.6,r16e0data[-1,0],1.5,100,200)*htoev
...
E0r117=np.zeros((4,len(zrange)))
E0r16 =np.zeros((4,len(zrange)))
E0z16 =np.zeros((4,len(rrange)))

Gamma  = 3.5
Nblist = [10,50,100,200]
for i in range(4):
    Nb = Nblist[i]
    #Get E0 for Gamma = 3.5 eV, BW = 7eV, Nb = 200
    E0r117[i,:] = [E0calc(1.17,zk,Gamma,7.0,Nb)*htoev for zk in zrange]
    E0r16[i,:]  = [E0calc(1.6,zk,Gamma,7.0,Nb)*htoev for zk in zrange]
    E0z16[i,:]  = [E0calc(rk,1.6,Gamma,7.0,Nb)*htoev for rk in rrange]

    #Calculate reference energy at R = 1.6 A and Z = 6.02 A 
    Eref = E0calc(1.6,r16e0data[-1,0],Gamma,7.0,Nb)*htoev

    #Set correct zero of energy for each calculation.
    E0r117[i,:] = [E0r117[i,k] - Eref + r16e0data[-1,1] for k in range(len(E0r117[i,:]))]
    E0r16[i,:]  = [E0r16[i,k] - Eref + r16e0data[-1,1] for k in range(len(E0r16[i,:]))]
    E0z16[i,:]  = [E0z16[i,k] - Eref + r16e0data[-1,1] for k in range(len(E0z16[i,:]))]
...
for i in range(4): 
    plt.plot(zrange,E0r117[i,:],'-',linewidth='3.0') 
#plt.plot(zrange,E0r117BW100,'-')
plt.title("R = 1.17 Å")
plt.xlabel("Z (Å)")
...
for i in range(4):
    plt.plot(zrange,E0r16[i,:],'-',linewidth='3.0') 
#plt.plot(zrange,E0r16BW100,'-')
plt.legend(["cDFT reference","N=10","N=50","N=100","N=200"],loc='upper right')
plt.title("R = 1.6 Å")
plt.xlabel("Z (Å)")
plt.ylabel("Energy (eV)")
#plt.rc('font', size=SMALL_SIZE)          # controls default text sizes
...
for i in range(4): 
    plt.plot(rrange,E0z16[i,:],'-',linewidth='3.0') 
#plt.plot(rrange,E0z16BW100,'-')
plt.title("Z = 1.6 Å")
plt.xlabel("R (Å)")
plt.ylabel("Energy (eV)")
#plt.rc('font', size=SMALL_SIZE)          # controls default text sizes
...
#np.savetxt("GHM_E0r117.csv",np.array([zrange,E0r117BW100]),delimiter = ",")
#np.savetxt("GHM_E0r16.csv",np.array([zrange,E0r16BW100]),delimiter = ",")
#np.savetxt("GHM_E0z16.csv",np.array([rrange,E0z16BW100]),delimiter = ",")
#np.savetxt("Gamm3p5_E0r117.csv",np.array([zrange,E0r117[3]]),delimiter = ",")
#np.savetxt("Gamm3p5_E0r16.csv",np.array([zrange,E0r16[3]]),delimiter = ",")
#np.savetxt("Gamm3p5_E0z16.csv",np.array([rrange,E0z16[3]]),delimiter = ",")
